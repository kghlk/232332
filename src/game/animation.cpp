...
void GetAnimationTransform(float GlobalTime, int Env, CLayers* pLayers, vec2& Position, float& Angle)
{
	...
	float Time = fmod(GlobalTime, pPoints[pItem->m_NumPoints - 1].m_Time.AsSeconds()) * 1000.0f;
	for(int i = 0; i < pItem->m_NumPoints-1; i++)
	{
		if(Time >= pPoints[i].m_Time.GetInternal() && Time <= pPoints[i + 1].m_Time.GetInternal())
		{
			float Delta = pPoints[i + 1].m_Time.GetInternal() - pPoints[i].m_Time.GetInternal();
			float a = (Time - pPoints[i].m_Time.GetInternal()) / Delta;
